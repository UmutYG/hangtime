...
export type MatchResult = {
  cardId: string | null;
  reason: string;
  // When the event is also living proof of one of the person's own named
  // strengths, its id plus one short reaffirmation line ("you said X about
  // yourself — that's exactly what you just did") for a stronger landing.
...
export async function matchEventToVision(
  apiKey: string,
  eventText: string,
  cards: VisionCard[],
  lang: Lang,
  ...
  const empty: MatchResult = { cardId: null, reason: "", strengthId: null, strengthLine: "" };
  if (cards.length === 0 && strengths.length === 0) return empty;
  ...
  const system =
    "You help a person practicing Reality Transurfing connect real-life positive " +
    "events to (a) the identity/vision they are growing into, and (b) the strengths " +
    "they already see in themselves.\n\n" +
    "VISION MATCH: given the event and a list of vision statements, decide whether " +
    "the event is genuine, specific evidence for ONE of the visions — meaning a " +
    "reasonable person would see a clear thematic link between this event and that " +
    "particular identity. Be discerning, not generous: a vague or merely pleasant " +
    "event that doesn't clearly belong to a specific vision must return null rather " +
    "than being force-fit to the nearest card. Only match when the link is real. " +
    "When you do match, the reason must name the concrete link in one warm sentence " +
    "spoken DIRECTLY TO the person — always 'you' ('sen'), never third person " +
    "('he/she/they' or their name); this line is said to their face, not written " +
    "about them in a report. " +
    "When nothing fits, return null with an empty reason.\n\n" +
    "STRENGTH REAFFIRMATION: independently, decide whether the event is a lived " +
    "example of ONE of the person's own named strengths (e.g. event 'I spoke up for " +
    "myself instead of avoiding the conflict' clearly embodies a strength like 'I am " +
    "comfortable in the uncomfortable'). Same discernment: only when the link is " +
    "obvious and real, else null with an empty line. When it matches, write ONE " +
    "short reaffirmation line that hands their own words back as proof — casual, " +
    "spoken, like a friend texting; think and write natively in the target " +
    "language, never translated-sounding. Calibration (match this register, never " +
    "reuse verbatim):\n" +
    "TR: \"'Rahatsızlıkta rahatım' demiştin ya — bugün tam olarak onu yaptın.\"\n" +
    "EN: \"You said 'I'm comfortable in the uncomfortable' — that's exactly what you just did.\"\n" +
    "No exclamation marks, under ~110 characters, no praise words like 'great job'.";

  const user =
    `Vision statements:\n${cardList || "(none)"}\n\n` +
    `Their named strengths:\n${strengthList || "(none)"}\n\n` +
    `Positive event: "${eventText}"\n\n` +
    `Return the vision id + one-sentence reason (or null), and the strength id + ` +
    `reaffirmation line (or null). Write all text in ${language}.`;
        ...
        format: {
          ...
          schema: {
            ...
            properties: {
              ...
              reason: { type: "string" },
              strengthId: { anyOf: [{ type: "string" }, { type: "null" }] },
              strengthLine: { type: "string" },
            },
            required: ["cardId", "reason", "strengthId", "strengthLine"],
  ...
  // Only trust ids that actually exist.
  const cardValid = cards.some((c) => c.id === parsed.cardId);
  const strengthValid = strengths.some((s) => s.id === parsed.strengthId);
  return {
    cardId: cardValid ? parsed.cardId : null,
    reason: cardValid ? parsed.reason ?? "" : "",
    strengthId: strengthValid ? parsed.strengthId : null,
    strengthLine: strengthValid ? (parsed.strengthLine ?? "").trim() : "",
  };
}

// Reframe something the user is seeing as negative, strictly along the book's
// own logic: locate whether the weight is inner importance (what others will
// think, who they'll be) or outer importance (the result itself); ask whether
// it would even affect them without that importance; then loosen it — everything
// is neutral, life is a game/scenario, there may be a positive sign not yet
// seen, and the only real moment is this one breath, which means there's a
// choice in how it's held.
export type ReframeResult = { reframe: string; positiveTakeaway: string };
...
export async function generateReframe(
  ...
  if (!apiKey) throw new NoApiKeyError();
  const language = lang === "tr" ? "Turkish" : "English";
  const anchors = [...byCategory("reframe"), ...byCategory("importance")]
    .map((c) => `- ${citationText(c, lang)}`)
    .join("\n");

  const system =
    "You help someone see something they're calling negative in a different " +
    "light. The ideas underneath come from Reality Transurfing, but you must " +
    "NEVER mention the book, cite it, or sound like you're reciting a concept. " +
    "These lines are only your own private grounding, not something to quote " +
    "or paraphrase visibly:\n" +
    anchors +
    "\n\nSpeak like a calm, plain-spoken friend thinking out loud — the register " +
    "of a good meditation app (think Waking Up), not a self-help book and not " +
    "therapy-speak. Ordinary words, short sentences, one flowing train of " +
    "thought — never a numbered or checklist structure, never a list of " +
    "'steps'. Let the ideas blend into each other the way a person actually " +
    "thinks, using their own answers as material where it fits naturally: " +
    "maybe it's more about how they'd look to others than about what actually " +
    "happened; maybe, without that worry, this wouldn't even register; maybe " +
    "none of it is as fixed or serious as it feels — more like a scene playing " +
    "out than a verdict; there might be something good in it they just haven't " +
    "spotted yet; and really, the past is just a memory and the future is a " +
    "guess your mind is making — right now, this breath, is the only thing " +
    "actually happening, and that's the one place a real choice exists. " +
    "Be as SHORT as it needs to be — usually 3 to 5 short sentences, each on " +
    "its own line, only going to 6 if it truly earns it. Second person, no " +
    "hype, no exclamation points, no invented facts about their situation. " +
    "Then write ONE separate short sentence, in the first person, present " +
    "tense, plain language, as something they've genuinely come to see — this " +
    "becomes their own logged positive sign (e.g. \"I'm learning that what " +
    "others think of me doesn't decide who I am\"). " +
    `CRITICAL: write everything in ${language}.`;

  const user =
    `What they're seeing as negative: "${negativeText}"\n` +
    `Why it matters so much to them: "${importanceAnswer || "(no answer given)"}"\n` +
    `Whether it would affect them without that importance: "${impactAnswer || "(no answer given)"}"\n\n` +
    `Write the reframe now, in ${language}.`;
      ...
      output_config: {
        format: {
          ...
          schema: {
            ...
            properties: {
              reframe: { type: "string" },
              positiveTakeaway: { type: "string" },
            },
            required: ["reframe", "positiveTakeaway"],
            additionalProperties: false,
          },
        },
      },
  ...
  try {
    ...
    return {
      reframe: tidyReflection(parsed.reframe ?? ""),
      positiveTakeaway: (parsed.positiveTakeaway ?? "").trim(),
    };
  } catch {
    return { reframe: "", positiveTakeaway: "" };
  }
}

// Short, warm, real-feeling reminders that gently turn attention to something
// genuinely good — like a kind friend, never a coach or an app.
